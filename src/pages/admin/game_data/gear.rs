...
impl Component for AdminGameDataGear {
    ...
    fn create(ctx: &Context<Self>) -> Self {
        ...
        let set_paging = ctx
            .link()
            .callback(AdminGameDataGearMessage::SetPagingStats);
    ...
    fn update(&mut self, ctx: &Context<Self>, msg: AdminGameDataGearMessage) -> bool {
        match msg {
                ...
                match self_editing_item {
                    Some(mut editing_item) => {
                        ...
                        let new_item_callback =
                            ctx.link().callback(AdminGameDataGearMessage::NewItem);

                        let update_gear_callback = ctx
                            .link()
                            .callback(AdminGameDataGearMessage::UpdateGearAndRefresh);
                ...
                for item in self.items.clone().into_iter() {
                    if item.id == id {
                        ...
                        let dialog = ConfirmationDialogDefinition {
                            ...
                            html: None,
                            text: Some(
                                "Are you sure you would like to delete the gear '".to_owned()
                                    + &item.name
                                    + &"'?",
                            ),
                            label_yes: None,
                ...
                for item in self.items.clone().into_iter() {
                    if item.id == id {
                        ...
                        let dialog = ConfirmationDialogDefinition {
                            ...
                            html: None,
                            text: Some(
                                "Are you sure you would like to duplicate the gear '".to_owned()
                                    + &item_name
                                    + &"'?",
                            ),
                            label_yes: None,
    ...
    fn view(&self, ctx: &Context<Self>) -> Html {
        let callback_fetch_admin_params = ctx
            .link()
            .callback(AdminGameDataGearMessage::SetFetchAdminParams)
            .clone();
        ...
        match &self.editing_item {
            Some(editing_item) => {
                ...
                if self.is_adding {
                    ...
                    add_callback = Some(
                        ctx.link()
                            .callback(AdminGameDataGearMessage::SaveItem)
                            .clone(),
                    );
                    save_and_leave_open_callback = Some(
                        ctx.link()
                            .callback(AdminGameDataGearMessage::SaveItemAndLeaveOpen)
                            .clone(),
                    );
                    read_only = false;
                }
