...
pub struct AdminGameDataGear {
    global_vars: GlobalVars,
    items: Vec<Gear>,
    paging_data: Option<AdminPagingStatistics>,
    paging_sorting_and_filter: FetchAdminParameters,
    loading: bool,
    editing_item: Option<Gear>,
    is_adding: bool,
    is_editing: bool,
}

impl Component for AdminGameDataGear {
    type Message = AdminGameDataGearMessage;
    type Properties = AdminGameDataGearProps;
    ...
    fn create(ctx: &Context<Self>) -> Self {
        ...
        let set_items = ctx.link().callback(AdminGameDataGearMessage::SetItems);
        let set_paging = ctx.link().callback(AdminGameDataGearMessage::SetPagingStats);
    ...
    fn update(
        &mut self,
        ctx: &Context<Self>,
        msg: AdminGameDataGearMessage
    ) -> bool {

        match msg {

            AdminGameDataGearMessage::ViewItem( id ) => {
            ...
            AdminGameDataGearMessage::EditItemDialog( id ) => {
                // self.editing_item = None;
                for item in self.items.clone().into_iter() {
                    if item.id == id {
                        self.editing_item = Some(item.clone());
            ...
            AdminGameDataGearMessage::AddItemDialog( _nv ) => {
                log!("AdminGameDataGearMessage::AddItemDialog");
                let mut new_hind = Gear::new();
            ...
            AdminGameDataGearMessage::SaveItem( as_new ) => {
                log!("AdminGameDataGearMessage::SaveItem");
                        ...
                        spawn_local (
                            async move {
                                let result = fetch_api_save_game_data_row(
                                    (api_root + "/admin/game-data/gear/save").to_owned(),
                                    req,
            ...
            AdminGameDataGearMessage::NewItem( book_id ) => {
                let self_editing_item = self.editing_item.clone();
                let mut hind = Gear::new();
                match self_editing_item {
                    Some( mut editing_item ) => {
            ...
            AdminGameDataGearMessage::SaveItemAndLeaveOpen( _unused ) => {
                log!("AdminGameDataGearMessage::SaveItemAndLeaveOpen");
                let self_editing_item = self.editing_item.clone();
                let self_is_adding = self.is_adding;
                ...
                match self_editing_item {
                    Some( mut editing_item ) => {
                        ...
                        // let item_name = editing_item.name.to_owned();
                        let set_items = ctx.link().callback(AdminGameDataGearMessage::SetItems);
                        let new_item_callback = ctx.link().callback(AdminGameDataGearMessage::NewItem);


                        let update_gear_callback = ctx.link().callback(AdminGameDataGearMessage::UpdateGearAndRefresh);
                                        ...
                                        match save_result {
                                            Ok( save_result_data) => {
                                                match save_result_data.game_data {
                                                    ...
                                                    update_gear_callback.emit( new_hind );
            ...
            AdminGameDataGearMessage::DeleteItem( id ) => {
                log!("AdminGameDataGearMessage::DeleteItem ", id);
                ...
                for item in self.items.clone().into_iter() {
                    if item.id == id {
                        ...
                        let dialog = ConfirmationDialogDefinition {
                            ...
                            html: None,
                            text: Some( "Are you sure you would like to delete the gear '".to_owned() + &item.name + &"'?" ),
                            label_yes: None,
                            label_no: None,
                                ...
                                spawn_local (
                                    async move {
                                        let result = fetch_api_delete_game_data_row(
                                            (api_root + "/admin/game-data/gear/delete").to_owned(),
                                            req,
                                        ).await;
            ...
            AdminGameDataGearMessage::DuplicateItem( id ) => {
                log!("AdminGameDataGearMessage::DuplicateItem", id);
                ...
                for item in self.items.clone().into_iter() {
                    if item.id == id {
                        ...
                        let dialog = ConfirmationDialogDefinition {
                            ...
                            html: None,
                            text: Some( "Are you sure you would like to duplicate the gear '".to_owned() + &item_name + &"'?" ),
                            label_yes: None,
                            label_no: None,
                            callback: Callback::from( move |_clicked_yes| {
                                                    ...
                                                    Ok( save_result_data) => {
                                                        match save_result_data.game_data {
                                                            Some( vec_val ) => {
                                                                ...
                                                                let alert_def: AlertDefinition = AlertDefinition {
                                                                    level: save_result_data.level,
                                                                    text: Some("Gear '".to_owned() + &item_name.to_owned() + &"' has been duplicated."),
                                                                    ..Default::default()
                                                                };
                                                                global_vars.add_alert.emit( alert_def );
            ...
            AdminGameDataGearMessage::Cancel( _new_value ) => {
                log!("AdminGameDataGearMessage::Cancel");
                self.editing_item = None;
            }

            AdminGameDataGearMessage::UpdateGear( new_value ) => {
                self.editing_item = Some(new_value);
                return false;

            }
            AdminGameDataGearMessage::UpdateGearAndRefresh( new_value ) => {
                self.editing_item = Some(new_value);
                return true;

            }

            AdminGameDataGearMessage::SetItems( new_value ) => {
                self.items = new_value;
                self.loading = false;
            }

            AdminGameDataGearMessage::SetPagingStats( new_value ) => {
            ...
            AdminGameDataGearMessage::SetFetchAdminParams( new_value ) => {
    ...
    fn view(
        ...
        let callback_fetch_admin_params = ctx.link().callback( AdminGameDataGearMessage::SetFetchAdminParams ).clone();
        let callback_fetch_admin_params_2 = ctx.link().callback( AdminGameDataGearMessage::SetFetchAdminParams ).clone();
        ...
        match &self.editing_item {
            Some( editing_item ) => {
                let mut editing_title = Some("Viewing Gear".to_owned());
                ...
                if self.is_adding {
                    editing_title = Some("Adding Gear".to_owned());
                    add_callback = Some(ctx.link().callback(AdminGameDataGearMessage::SaveItem).clone());
                    save_and_leave_open_callback = Some(ctx.link().callback(AdminGameDataGearMessage::SaveItemAndLeaveOpen).clone());
                    read_only = false;
                }

                if self.is_editing {
                    editing_title = Some("Editing Gear".to_owned());
                    save_callback = Some(ctx.link().callback(AdminGameDataGearMessage::SaveItem).clone());
                    save_as_new_callback = Some(ctx.link().callback(AdminGameDataGearMessage::SaveItem).clone());
                    save_and_leave_open_callback = Some(ctx.link().callback(AdminGameDataGearMessage::SaveItemAndLeaveOpen).clone());
                    read_only = false;
                }
                ...
                <StandardModal
                    xl={true}
                    title={editing_title}
                    close_cancel_callback={Some(ctx.link().callback(AdminGameDataGearMessage::Cancel).clone())}
                ...
                >
                    <EditGear
                        ...
                        book_list={book_list}
                        on_changed_callback={ctx.link().callback(AdminGameDataGearMessage::UpdateGear).clone()}
                    />
        ...
        let add_item = ctx.link().callback(AdminGameDataGearMessage::AddItemDialog);

        html! {
        <UIPage
            global_vars={global_vars.clone()}
            page_title="Admin Gear"
        ...
        <div class="pull-right">
            ...
            />
                ...
                <h2><i class="fa fa-items" /><Nbsp />{"Admin Gear"}</h2>
                        ...
                        if self.loading {
                            ...
                            {self.items.clone().into_iter().map( move |row| {
                                ...
                                if global_vars.current_user.admin_can_read_item (
                                    ...
                                    callback_view_item = Some(ctx.link().callback(AdminGameDataGearMessage::ViewItem));
                                }
                                if global_vars.current_user.admin_can_write_item (
                                    ...
                                    callback_edit_item = Some(ctx.link().callback(AdminGameDataGearMessage::EditItemDialog));
                                    callback_duplicate_item = Some(ctx.link().callback(AdminGameDataGearMessage::DuplicateItem));
                                }
                                if global_vars.current_user.admin_can_delete_item (
                                    ...
                                    callback_delete_item = Some(ctx.link().callback(AdminGameDataGearMessage::DeleteItem));
                                }
                                html!{<tr>
...
async fn _get_data(
    ...
    let result = fetch_admin_api(
        (api_root + "/admin/game-data/gear/paging").to_owned(),
        paging_sorting_and_filter.clone(),
