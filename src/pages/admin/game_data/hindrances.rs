...
impl Component for AdminGameDataHindrances {
    ...
    fn create(ctx: &Context<Self>) -> Self {
        ...
        let set_paging = ctx
            .link()
            .callback(AdminGameDataHindrancesMessage::SetPagingStats);
    ...
    fn update(&mut self, ctx: &Context<Self>, msg: AdminGameDataHindrancesMessage) -> bool {
        match msg {
                ...
                match self_editing_item {
                    Some(mut editing_item) => {
                        ...
                        let new_item_callback =
                            ctx.link().callback(AdminGameDataHindrancesMessage::NewItem);

                        let update_hindrance_callback = ctx
                            .link()
                            .callback(AdminGameDataHindrancesMessage::UpdateHindranceAndRefresh);
                ...
                for item in self.items.clone().into_iter() {
                    if item.id == id {
                        ...
                        let dialog = ConfirmationDialogDefinition {
                            ...
                            html: None,
                            text: Some(
                                "Are you sure you would like to delete the hindrance '".to_owned()
                                    + &item.name
                                    + &"'?",
                            ),
                            label_yes: None,
                            ...
                            callback: Callback::from(move |_clicked_yes| {
                                ...
                                spawn_local(async move {
                                    let result = fetch_api_delete_game_data_row(
                                        (api_root + "/admin/game-data/hindrances/delete")
                                            .to_owned(),
                                        req,
                                    )
                                    .await;
                ...
                for item in self.items.clone().into_iter() {
                    if item.id == id {
                        ...
                        let dialog = ConfirmationDialogDefinition {
                            ...
                            html: None,
                            text: Some(
                                "Are you sure you would like to duplicate the hindrance '"
                                    .to_owned()
                                    + &item_name
                                    + &"'?",
                            ),
    ...
    fn view(&self, ctx: &Context<Self>) -> Html {
        let callback_fetch_admin_params = ctx
            .link()
            .callback(AdminGameDataHindrancesMessage::SetFetchAdminParams)
            .clone();
        ...
        match &self.editing_item {
            Some(editing_item) => {
                ...
                if self.is_adding {
                    ...
                    add_callback = Some(
                        ctx.link()
                            .callback(AdminGameDataHindrancesMessage::SaveItem)
                            .clone(),
                    );
                    save_and_leave_open_callback = Some(
                        ctx.link()
                            .callback(AdminGameDataHindrancesMessage::SaveItemAndLeaveOpen)
                            .clone(),
                    );
                    read_only = false;
                }
        ...
        let add_item = ctx
            .link()
            .callback(AdminGameDataHindrancesMessage::AddItemDialog);
            ...
            match vec_val_result {
                Ok(vec_val) => {
                    ...
                    for data in vec_val.into_iter() {
                        let item = data.to_hindrance().unwrap();
                        // log!(format!("hind {} {}", &item.name, mem::size_of_val(&item)));
                        rv.push(item);
                    }
