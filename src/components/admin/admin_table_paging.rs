...
pub fn edit_view_delete_buttons(
    ...
    let callback_set_page_count = Callback::from(
        move |e: Event | {
            e.prevent_default();
    ...
    let callback_set_current_page= Callback::from(
        move |e: Event | {
            e.prevent_default();
    ...
    let callback_next_page= Callback::from(
        move |e: MouseEvent | {
            e.prevent_default();
    ...
    let callback_previous_page= Callback::from(
        move |e: MouseEvent | {
            e.prevent_default();
    ...
    match &props.stats {
        Some( paging_stats) => {
            let number_of_pages = round::ceil( (paging_stats.filtered_count / props.paging_sorting_and_filter.number_per_page) as f64, 0) as u32;
