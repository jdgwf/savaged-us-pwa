...
pub struct ConflictsEntryProps {
    #[prop_or_default]
...
pub enum ConflictsEntryMessage {
    OnChange(String),
}

pub struct ConflictsEntry;

impl Component for ConflictsEntry {
    type Message = ConflictsEntryMessage;
    type Properties = ConflictsEntryProps;

    fn create(_ctx: &Context<Self>) -> Self {
        ConflictsEntry {}
    }

    fn update(&mut self, ctx: &Context<Self>, msg: ConflictsEntryMessage) -> bool {
        match msg {
            ConflictsEntryMessage::OnChange(new_value) => {
    ...
    fn view(&self, ctx: &Context<Self>) -> Html {
        ...
        let onchange = ctx.link().callback(|event: InputEvent| {
            ...
            ConflictsEntryMessage::OnChange(input.value())
        });
