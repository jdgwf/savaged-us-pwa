...
pub struct AbilitiesEntryProps {
    #[prop_or_default]
...
pub enum AbilitiesEntryMessage {
    OnChange(String),
}

pub struct AbilitiesEntry;

impl Component for AbilitiesEntry {
    type Message = AbilitiesEntryMessage;
    type Properties = AbilitiesEntryProps;

    fn create(_ctx: &Context<Self>) -> Self {
        AbilitiesEntry {}
    }

    fn update(&mut self, ctx: &Context<Self>, msg: AbilitiesEntryMessage) -> bool {
        match msg {
            AbilitiesEntryMessage::OnChange(new_value) => {
    ...
    fn view(&self, ctx: &Context<Self>) -> Html {
        ...
        let onchange = ctx.link().callback(|event: InputEvent| {
            ...
            AbilitiesEntryMessage::OnChange(input.value())
        });
