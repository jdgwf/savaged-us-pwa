...
pub async fn update_current_user(
    ...
    match current_user_convert {
        Ok(value) => {
            ...
            let post_val_convert: Result<serde_json::Value, serde_json::Error> =
                serde_json::from_str(&current_user_string.as_ref());
            match post_val_convert {
                Ok(mut post_val) => {
                    ...
                    post_val["repeat_password"] =
                        serde_json::Value::String(repeat_password.to_owned());
                    post_val["remove_image"] = serde_json::Value::Bool(remove_image);
                    post_val["current_user"] =
                        serde_json::Value::String(current_user_string.clone());

                    let post_val_string = post_val.to_string();

                    let post_value = &wasm_bindgen::JsValue::from_str(&post_val_string);

                    opts.body(Some(post_value));
                    opts.mode(RequestMode::Cors);
                    let request = Request::new_with_str_and_init(&endpoint, &opts)?;
...
pub async fn fetch_api_for_id_with_value(
    ...
    opts.body(Some(&wasm_bindgen::JsValue::from_str(
        format!(
            ...
            &login_token, &remote_id_name, &id, &new_value_name, &new_value,
        )
        .as_ref(),
    )));
    opts.mode(RequestMode::Cors);
...
pub async fn upload_user_image(
    ...
    match resp_result {
        Ok(resp) => {
            ...
            let parsed_result: Result<FileUploadResults, Error> =
...
pub async fn fetch_api_with_value(
    ...
    opts.body(Some(&wasm_bindgen::JsValue::from_str(
        format!(
            "{{\"api_key\": \"{}\", \"login_token\": \"{}\", \"{}\": \"{}\"}}",
            &api_key, &login_token, &var_name, &var_value
        )
        .as_ref(),
    )));
    opts.mode(RequestMode::Cors);
...
pub async fn fetch_api_send_token(
    ...
    opts.body(Some(&wasm_bindgen::JsValue::from_str(
        format!(
            ...
            &login_token, &selected_token, &new_value,
        )
        .as_ref(),
    )));
    opts.mode(RequestMode::Cors);
